...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        return (self.startingPosition, tuple(self.corners)) # tuple ((x, y) (corner1, corner2 ...))

    def isGoalState(self, state):
        """
        Returns whether this search state is a goal state of the problem.
        """
        return state[1] == () # if no more corners to cover, we've reached the goal stater
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall: # action is valid
                nextCorners = []
                for corner in state[1]:
                    if corner != (nextx, nexty): # if the next move does not cover the corner
...
def cornersHeuristic(state, problem):
    ...
    corners = state[1]
    if corners != ():
        return max([util.manhattanDistance(state[0], corner) for corner in corners])
    else:
        return 0
